...
```console
$ docker pull solr@sha256:bd74a7c4801bd713c7b792299651ba3d9bf7c5d452c29d5d6771db9dbb2801f7
```
...
```console
$ docker pull solr@sha256:48e05927b379b4f67ff9b8ec070b3eb637d9389d487f1b414a0b358528475ff2
```

-	Docker Version: 17.06.0-ce
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **349.2 MB (349211693 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:170ca216cf27e1c42bc683889cbbcaf872a0208221317b68846c6fb3655c5620`
-	Entrypoint: `["docker-entrypoint.sh"]`
-	Default Command: `["solr-foreground"]`
...
RUN mkdir -p /opt/solr &&   echo "downloading $SOLR_URL" &&   wget -nv $SOLR_URL -O /opt/solr.tgz &&   echo "downloading $SOLR_URL.asc" &&   wget -nv $SOLR_URL.asc -O /opt/solr.tgz.asc &&   echo "$SOLR_SHA256 */opt/solr.tgz" | sha256sum -c - &&   (>&2 ls -l /opt/solr.tgz /opt/solr.tgz.asc) &&   gpg --batch --verify /opt/solr.tgz.asc /opt/solr.tgz &&   tar -C /opt/solr --extract --file /opt/solr.tgz --strip-components=1 &&   rm /opt/solr.tgz* &&   rm -Rf /opt/solr/docs/ &&   mkdir -p /opt/solr/server/solr/lib /opt/solr/server/solr/mycores /opt/solr/server/logs /docker-entrypoint-initdb.d /opt/docker-solr &&   sed -i -e 's/"\$(whoami)" == "root"/$(id -u) == 0/' /opt/solr/bin/solr &&   sed -i -e 's/lsof -PniTCP:/lsof -t -PniTCP:/' /opt/solr/bin/solr &&   sed -i -e 's/#SOLR_PORT=8983/SOLR_PORT=8983/' /opt/solr/bin/solr.in.sh &&   sed -i -e '/-Dsolr.clustering.enabled=true/ a SOLR_OPTS="$SOLR_OPTS -Dsun.net.inetaddr.ttl=60 -Dsun.net.inetaddr.negative.ttl=60"' /opt/solr/bin/solr.in.sh &&   chown -R $SOLR_USER:$SOLR_GROUP /opt/solr
# Fri, 22 Dec 2017 14:24:36 GMT
COPY dir:5fd6e310972599026a88a8cba1cf0f73243ea6fab4a0bb77f6483c1dddc64d6e in /opt/docker-solr/scripts 
# Fri, 22 Dec 2017 14:24:36 GMT
RUN chown -R $SOLR_USER:$SOLR_GROUP /opt/docker-solr
# Fri, 22 Dec 2017 14:24:37 GMT
EXPOSE 8983/tcp
# Fri, 22 Dec 2017 14:24:37 GMT
WORKDIR /opt/solr
# Fri, 22 Dec 2017 14:24:37 GMT
...
-	Layers:
	...
	-	`sha256:c4ee140eb5154b8c56a0e15a9930de77b0221b98da77b54f505abea963c84308`  
		...
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:69b341122e033837bd9ac5cec1e3a814b10ed739e3b54466999b8fca44af306b`  
		Last Modified: Fri, 22 Dec 2017 14:28:06 GMT  
		Size: 4.2 KB (4157 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:1c6339b147b2b74e27917c3cb2c8fc66ed7c326481d4c092fe7dc977797e017c`  
		Last Modified: Fri, 22 Dec 2017 14:28:06 GMT  
		Size: 4.1 KB (4133 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
...
```console
$ docker pull solr@sha256:b93cd374449d10e8ecf9046914a39f6ed7bb8184dea7873d4815a8c0f72b020c
```

-	Docker Version: 17.06.2-ce
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **360.5 MB (360520465 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:18a1ebdf27498a40c16341e993e20ba55ded8cba760ae8f12c520a672d04123a`
-	Entrypoint: `["docker-entrypoint.sh"]`
-	Default Command: `["solr-foreground"]`
...
RUN mkdir -p /opt/solr &&   echo "downloading $SOLR_URL" &&   wget -nv $SOLR_URL -O /opt/solr.tgz &&   echo "downloading $SOLR_URL.asc" &&   wget -nv $SOLR_URL.asc -O /opt/solr.tgz.asc &&   echo "$SOLR_SHA256 */opt/solr.tgz" | sha256sum -c - &&   (>&2 ls -l /opt/solr.tgz /opt/solr.tgz.asc) &&   gpg --batch --verify /opt/solr.tgz.asc /opt/solr.tgz &&   tar -C /opt/solr --extract --file /opt/solr.tgz --strip-components=1 &&   rm /opt/solr.tgz* &&   rm -Rf /opt/solr/docs/ &&   mkdir -p /opt/solr/server/solr/lib /opt/solr/server/solr/mycores /opt/solr/server/logs /docker-entrypoint-initdb.d /opt/docker-solr &&   sed -i -e 's/"\$(whoami)" == "root"/$(id -u) == 0/' /opt/solr/bin/solr &&   sed -i -e 's/lsof -PniTCP:/lsof -t -PniTCP:/' /opt/solr/bin/solr &&   sed -i -e 's/#SOLR_PORT=8983/SOLR_PORT=8983/' /opt/solr/bin/solr.in.sh &&   sed -i -e '/-Dsolr.clustering.enabled=true/ a SOLR_OPTS="$SOLR_OPTS -Dsun.net.inetaddr.ttl=60 -Dsun.net.inetaddr.negative.ttl=60"' /opt/solr/bin/solr.in.sh &&   chown -R $SOLR_USER:$SOLR_GROUP /opt/solr
# Fri, 22 Dec 2017 15:07:05 GMT
COPY dir:5fd6e310972599026a88a8cba1cf0f73243ea6fab4a0bb77f6483c1dddc64d6e in /opt/docker-solr/scripts 
# Fri, 22 Dec 2017 15:07:08 GMT
RUN chown -R $SOLR_USER:$SOLR_GROUP /opt/docker-solr
# Fri, 22 Dec 2017 15:07:10 GMT
EXPOSE 8983/tcp
# Fri, 22 Dec 2017 15:07:11 GMT
WORKDIR /opt/solr
# Fri, 22 Dec 2017 15:07:12 GMT
USER [solr]
# Fri, 22 Dec 2017 15:07:13 GMT
ENTRYPOINT ["docker-entrypoint.sh"]
# Fri, 22 Dec 2017 15:07:14 GMT
CMD ["solr-foreground"]
```
...
-	Layers:
	...
	-	`sha256:6c04c1f7d8fcc8688535676a0b043faa7a4ebd9e8e57ab5f2e8f36b7f0fd400b`  
		...
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:d55f4b24443a55773c62bd72b45758eb0bf34cde345c21b314135a59ab8e86de`  
		Last Modified: Fri, 22 Dec 2017 15:14:35 GMT  
		Size: 4.2 KB (4163 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:02cdfb0769595beaa35ddf8ba564a2a4b7d249eebb161ec951cd35375369b1ce`  
		Last Modified: Fri, 22 Dec 2017 15:14:35 GMT  
...
```console
$ docker pull solr@sha256:3312cdd054907e356e78a9145ce717b0b71ccbdf4775cacec0d901baf172ac9c
```

-	Docker Version: 17.06.1-ce
-	Manifest MIME: `application/vnd.docker.distribution.manifest.v2+json`
-	Total Size: **342.4 MB (342404927 bytes)**  
	(compressed transfer size, not on-disk size)
-	Image ID: `sha256:af919991defb5baa5a034dde088a06e4ec1951b0609a1dd7923561eea3704a46`
-	Entrypoint: `["docker-entrypoint.sh"]`
-	Default Command: `["solr-foreground"]`
...
RUN mkdir -p /opt/solr &&   echo "downloading $SOLR_URL" &&   wget -nv $SOLR_URL -O /opt/solr.tgz &&   echo "downloading $SOLR_URL.asc" &&   wget -nv $SOLR_URL.asc -O /opt/solr.tgz.asc &&   echo "$SOLR_SHA256 */opt/solr.tgz" | sha256sum -c - &&   (>&2 ls -l /opt/solr.tgz /opt/solr.tgz.asc) &&   gpg --batch --verify /opt/solr.tgz.asc /opt/solr.tgz &&   tar -C /opt/solr --extract --file /opt/solr.tgz --strip-components=1 &&   rm /opt/solr.tgz* &&   rm -Rf /opt/solr/docs/ &&   mkdir -p /opt/solr/server/solr/lib /opt/solr/server/solr/mycores /opt/solr/server/logs /docker-entrypoint-initdb.d /opt/docker-solr &&   sed -i -e 's/"\$(whoami)" == "root"/$(id -u) == 0/' /opt/solr/bin/solr &&   sed -i -e 's/lsof -PniTCP:/lsof -t -PniTCP:/' /opt/solr/bin/solr &&   sed -i -e 's/#SOLR_PORT=8983/SOLR_PORT=8983/' /opt/solr/bin/solr.in.sh &&   sed -i -e '/-Dsolr.clustering.enabled=true/ a SOLR_OPTS="$SOLR_OPTS -Dsun.net.inetaddr.ttl=60 -Dsun.net.inetaddr.negative.ttl=60"' /opt/solr/bin/solr.in.sh &&   chown -R $SOLR_USER:$SOLR_GROUP /opt/solr
# Sun, 07 Jan 2018 09:34:44 GMT
COPY dir:5fd6e310972599026a88a8cba1cf0f73243ea6fab4a0bb77f6483c1dddc64d6e in /opt/docker-solr/scripts 
# Sun, 07 Jan 2018 09:34:44 GMT
RUN chown -R $SOLR_USER:$SOLR_GROUP /opt/docker-solr
# Sun, 07 Jan 2018 09:34:48 GMT
EXPOSE 8983/tcp
# Sun, 07 Jan 2018 09:34:48 GMT
WORKDIR /opt/solr
# Sun, 07 Jan 2018 09:34:48 GMT
...
-	Layers:
	...
	-	`sha256:35c0057372c781ce56becbac30af3a4b3cda9ec27bdad40eb347904010b038d4`  
		...
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:62360e1a9facf7b2dc18fbb3bf8345b494b00ae0ca8e0b0ebeab4669b1399216`  
		Last Modified: Sun, 07 Jan 2018 09:42:32 GMT  
		Size: 4.1 KB (4130 bytes)  
		MIME: application/vnd.docker.image.rootfs.diff.tar.gzip
	-	`sha256:834c6a2a2036d01b095951b94afc711bb673478b2b9d35d31c7c19dabdcb6c14`  
		Last Modified: Sun, 07 Jan 2018 09:42:33 GMT  
